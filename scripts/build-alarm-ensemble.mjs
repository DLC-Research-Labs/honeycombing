...
const UNIT_REFERENCE_PLAN = "nc-2025-enacted-congressional";
...
// district across all plans, and key each district of the 2025 enacted plan
// (SL 2025-95, the map in force) by the rank it holds within its own plan.
...
const unitPlanSorted = [...unitPlan].sort((a, b) => a.share - b.share);
// Adjacent ranks separated by less than the calibration bound are not robust
// to the centroid shortcut: exact assignment could swap them. Disclosed below.
const rankTiePairs = unitPlanSorted
  .slice(1)
  .map((district, index) => ({ a: unitPlanSorted[index], b: district }))
  .filter(({ a, b }) => b.share - a.share < maxCalibrationDelta)
  .map(({ a, b }) => `${a.districtId} and ${b.districtId}`);
const rankedUnits = unitPlanSorted
  .map((district, rank) => {
...
const summary = {
  ...
  unitMeasures: [
    {
      ...
      definition:
        ...
        + "where k is the rank each 2025 enacted (SL 2025-95) district holds within its own plan. Districts are "
        + "compared by rank, not identity, because district numbering is not stable across plans.",
  ...
  caveats: [
    ...
    "The district-level unit measure (ranked_dem_share_pre20) ranks the districts of the 2025 enacted plan "
      + "(SL 2025-95, the map in force for 2026); the superseded 2023 plan appears in comparedPlans only. "
      + "District rank assignments inherit the centroid-assignment calibration bound"
      + (rankTiePairs.length > 0
        ? `; districts ${rankTiePairs.join("; ")} differ by less than that bound, so their ranks (and unit percentiles) could swap under exact assignment.`
        : "."),
