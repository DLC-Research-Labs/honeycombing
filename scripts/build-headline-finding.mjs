...
const GENERATED_AT = "2026-07-08T00:00:00Z";
...
const headline =
  `${enacted.value} of ${districtCount} districts lean Democratic under North Carolina's 2023 enacted congressional `
  + `map — ${plansAbovePct}% of ${planCount.toLocaleString("en-US")} neutral simulated maps produce more.`;

const shortHeadline =
  `NC's 2023 enacted map: ${enacted.value} of ${districtCount} Democratic-leaning districts — `
...
const methodNote =
  ...
  + `simulated ensemble (redist SMC, NC constraints). Ensemble median: ${medianSeats} Democratic-leaning seats; the enacted plan sits at `
  + `the ${percentile}th percentile. Diagnostic position, not a seat forecast and not legal evidence.`;
