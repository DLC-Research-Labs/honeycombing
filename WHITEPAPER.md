...
Rigor about whether an enacted district map is a statistical outlier comes from redistricting *ensembles* — large collections of computer-drawn legal maps that establish what a map of a state drawn to a documented set of districting rules (neutral in that it optimizes no partisan goal) tends to look like. The ALARM Project's 5,000 simulated North Carolina plans, and the MGGG/GerryChain and Duke *Quantifying Gerrymandering* ensembles, are the state of the art. Their weakness is legibility: an ensemble's statistical output is hard for a journalist, advocate, or judge to *see*.
...
Applied to North Carolina's congressional map adopted in October 2025 for the 2026 election (Session Law 2025-95), the ALARM ensemble places the enacted map at an extreme: 3 of its 14 districts lean Democratic on the 2020 presidential vote proxy — at or below the minimum of 5,000 simulated maps, of which only about 0.06% produce so few Democratic seats. That position is `descriptive` relative to the ensemble's documented constraint set; it is not, by itself, evidence of unlawful intent or legal injury, and it rests on the most conservative proxy available: of the eleven statewide-election proxies ALARM ships, neutral simulated maps reach so few Democratic seats most often under the presidential proxy we use, making it the least favorable to the outlier reading.
...
North Carolina redrew its congressional districts mid-decade in October 2025 (Session Law 2025-95) for the 2026 election. Run against the ALARM Project's ensemble of 5,000 simulated North Carolina plans, the enacted map is a stark outlier: **3 of its 14 districts lean Democratic** on the 2020 presidential proxy, which lands it **at or below the minimum of the entire ensemble** — only 3 of the 5,000 simulated maps (≈0.06%, 95% CI 0.02–0.18%) produce so few Democratic-leaning seats. Those three tail plans come from just one of ALARM's two independent simulation runs; the other run produced no plan below four seats, so the robust, chain-independent statement is that the enacted map sits *at or below the simulated minimum in both runs*. The ensemble's median is 6. The map it replaced (SL 2023-145, used in the 2024 election) had 4 Democratic-leaning districts and sat at the 2.8th percentile of the same ensemble.

Three things make this finding honest rather than rhetorical:

- **It rests on the ensemble, not the hexes.** Both the ensemble distribution and the enacted map's seat count are computed at the district level — the ensemble from ALARM's per-district vote totals, the enacted map from precinct-centroid assignment calibrated to ALARM's exact assignment within 0.12 percentage points. The H3 grid is how the pattern is *shown*, not how it is *measured*; the number is identical at any hex resolution. `descriptive`
- **It is re-derivable from published data.** Every number comes from ALARM's public statistics (Harvard Dataverse, doi:10.7910/DVN/SLCD3E, CC0); the finding is not new science, it is a legible presentation of what the ensemble already implies. It is consistent with the outlier findings that independent ensembles such as Duke's *Quantifying Gerrymandering* have reported for prior North Carolina maps, and the same pipeline's characterization of the prior map (SL 2023-145) matched the actual 2024 congressional result (10 Republican, 4 Democratic seats).
- **Our proxy is the conservative choice.** Democratic "seats" use the 2020 presidential vote as a partisan-lean proxy. Of the eleven statewide-election proxies ALARM ships, the presidential proxy produces the *widest* low tail among neutral simulated maps — under every 2020 statewide race and ALARM's composite, few-Democratic-seat maps are rarer, so presidential is the most conservative basis for the outlier call. (This compares the ensemble tails; our VEST precinct data carries only the presidential proxy, so we do not re-score the enacted map under the other elections.) Full table in §5.5.
...
This national projection is deliberately kept as an early, exploratory signal — and it is exactly the kind of number the reframed tool no longer leans on. Raw hex counts as a seat proxy are sensitive to resolution and to the equal-area-not-equal-population problem (the modifiable areal unit problem); the D proxy visibly swings with resolution in the table above. **Unlike every other quantitative claim in this paper, the figures in this table are not backed by a tracked reproducibility artifact** — treat them as an illustration of the aggregation problem, not as a result. The North Carolina finding in §3.1 avoids this entirely by computing seats at the district level against an ensemble rather than by counting hexes. Where a claim has to be rigorous, it comes from the ensemble; the hexes stay in their role as the picture.
...
### 4.3 The Voting Rights Act (post-*Callais*)

The Voting Rights Act has historically required that protected minority communities not have their electoral power diluted, and legally required opportunity districts may take boundaries that are not compact or grid-like. That landscape shifted sharply in *Louisiana v. Callais* (decided April 29, 2026): the Supreme Court held Louisiana's second majority-minority district an unconstitutional racial gerrymander and that §2 did not require it, narrowing §2 vote-dilution claims by demanding that plaintiffs' illustrative maps satisfy all of a state's legitimate districting objectives, including its political goals. Any VRA reading in this paper should be understood as post-*Callais* and unsettled.

Two consequences for the North Carolina finding, stated plainly:

- **The finding is partisan, not racial, and reaches neither lane a court can currently act on.** *Rucho* closed the federal partisan-gerrymandering door; the live challenge to SL 2025-95 is instead a racial vote-dilution / racial-gerrymander case (CD-1's Black voting-age population fell from roughly 40% to 32% in the redraw). Our ensemble measures a *partisan* seat outlier. It has no bearing on the racial claims actually pending, and it cannot show whether any deviation was legally required — that needs CVAP, racially-polarized-voting, and candidate-of-choice evidence this tool does not have.
- **The ensemble's race constraint is a 2020-cycle baseline, and its direction runs against the map, not for it.** ALARM's simulations hold the number of majority-minority districts near the enacted 2022 reference plan (a hinge-Gibbs simulation constraint, not a VRA §2 analysis), which predates both the 2025 redraw and *Callais*. Majority-minority districts pack heavily-Democratic Black voters into a few seats, which *suppresses* the Democratic seat count elsewhere — so the neutral baseline is already more Republican-favorable, and the map's 3-seat outlier persists *net of* whatever minority-opportunity structure the ensemble builds in. The map cannot be explained away as VRA compliance.

For the prescriptive research track, the lesson is unchanged but the target is moving: show a neutral scaffold, then document where and why legal constraints require departures from it.
...
- **Duke Quantifying Gerrymandering (Mattingly, Herschlag, et al.)**: independent ensemble analysis of North Carolina congressional maps.
...
Neither. The ensemble is the ALARM Project's public dataset (Harvard Dataverse, CC0). Independent groups — notably Duke's *Quantifying Gerrymandering* — have found prior North Carolina maps to be outliers using their own ensembles. Honeycombing's contribution is the legible presentation, not the underlying statistics.
